...
public class ClientGUI extends javax.swing.JFrame implements Observer, ChatIF{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5566687549466754928L;
	public ChatClient ch;
    public OpenDrawPad paint;
...
//					"Error", JOptionPane.ERROR_MESSAGE);
    ...
    private void PmOptionSelected(ActionEvent evt) {
    	PMFrame chan = PMFrame.getInstance(this);
        Point p = MouseInfo.getPointerInfo().getLocation();
        int xOffset = (int) p.getX() - 25;
        int yOffset = (int) p.getY() - 25;
        chan.setLocation(xOffset, yOffset);
		...
		if (arg instanceof serverNotification) {
			...
			if (paint != null) {
				...
				if (tokens[0].equals("#send")) {
					try{
						ch.sendToServer(tokens[1]);
					}catch(IOException e){
						e.printStackTrace();
					}
				}else if(tokens[0].startsWith("#linedraw")){
					paint.update(ch, arg);
				}
			}
		}
